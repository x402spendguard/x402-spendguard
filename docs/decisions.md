...
**Tests (failing-first).** `rejects-world-writable-policy-dir` (CONF-03), `ledger-refuses-world-writable-dir` (ACCT-08), `ledger-refuses-world-writable-dir-even-if-sticky` (the sticky-blind fork) — all RED before the checks, GREEN after; all self-skip on win32 (PLAT-01), like the file-level perm tests. No new predicate: both call sites reuse `modeIsWorldWritable`. Held for CHENG's adversarial pass before merge.
