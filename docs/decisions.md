...
**Decision.** The enforcement path contains **no security-deciding literal** — no hardcoded skew, no threshold, no default toggle. Every number, list, and tolerance is read from the user's `Policy`. Shipped defaults (e.g. clock-skew, the cross-origin flag) live in a **readable starter policy file** (`policy.example.json`, shipped as code via `STARTER_POLICY_JSON`; see ONBOARD-01), never as constants in code. A reader of the guard's code should find zero numbers that decide an outcome.
...
**2026-07-12 · Accepted (architecture; implementation is the next slice)**
...
**Status.** Built + green (default gate **121** = 119 + 2 fs-perms tests; tsc clean; **0 runtime deps**; no-egress static proof intact — `fs-perms.ts` imports nothing, only reads `process.platform`; e2e unchanged 12 + teeth). Requirements-first: PLAT-01 added (traceability green). Failing-test-first: a stub `modeIsWorldWritable` (unguarded) → the win32 unit + `perm-gates-skipped-on-windows` integration tests RED (the refusal fired under faked `win32`) → added the guard → GREEN. Platform faked via `Object.defineProperty(process, "platform", …)` with restore. Held for Opus's adversarial pass before merge. **→ Merged (`7a887b4`, Opus GO — he came in expecting fail-closed, agreed the check measures noise-not-danger on Windows).**

### D-036 — verifiable audit log: a tamper-EVIDENT hash chain (integrity core)
**2026-07-16 · Accepted (read-API ladder rung; the trust-model doctrine applied to the log)**

**Context.** The decision log (AS3) is the forensic record and the read-API's tightest rung ([[dashboard-positioning]]). This slice makes it **tamper-evident**: per-entry `seq` + hash chain (each record commits to the prior). Designed through the ratified trust model (THREAT_MODEL §3) and Opus's shape review, which added two load-bearing tightenings before the build.

**Decision — a hash chain with an injected integrity seam, framed as detection-not-prevention.**
- **`ChainHasher` seam (AUDIT-02, `src/audit/chain-hasher.ts`)** — the first-class injectable integrity primitive: default **unkeyed SHA-256**, pluggable to keyed **HMAC-SHA-256** with no core change. This is the key/anchor seam the trust-model reversibility analysis mandated: a future shared-host deployment (or a ledger MAC) **reuses this exact contract** — reversal is reuse, not rebuild. `node:crypto` (stdlib, DEP-01 intact; not network-capable, no-egress intact).
- **`HashChainDecisionLog` (AUDIT-01/03, `src/audit/hash-chain-log.ts`)** — append-only JSONL of `{seq, prev, hash, alg, entry}` envelopes; `entry` is the existing PRIV-02-clean `LogEntry` (chain commits to **contents**, never a secret). `verify({expectedHead?})` walks + re-derives each hash, checks linkage + seq contiguity, flags unparseable lines and genesis-at-nonzero (discontinuity).
- **`LoggingGuard` gains an injected `onAuditFailure`** — a swallowed sink is no longer invisible (AUDIT-03), while the verdict stays structurally independent (FAIL-03): the notifier runs after the decision and its own failure is ignored.

**The framing that makes this safe (the crux).** The audit log is **forensic, not enforcement** — per FAIL-03 its integrity failure **never flips a verdict**, so a broken chain **surfaces loud but never gates a payment** and thus **can never become a DoS**. This is the honest resolution of Opus's carry-forward recovery concern: the ledger MAC would fail-*closed* (DoS on benign corruption); the audit chain fails-*loud-but-open*. Same detect-then-surface primitive, opposite blast radius, because one gates money and the other gates a record.

**Opus's two shape tightenings, resolved + pinned as tests:**
1. **`verify()` is anchor-relative — "tamper-evident" must not overclaim.** An unkeyed self-consistent full rewrite passes self-verify (public hash → attacker recomputes forward). So the honest matrix is documented, and self-verify is a **health check** (corruption + naive tamper), NOT an anti-rewrite proof — anti-rewrite needs an **external `expectedHead`** (in the type signature, unmissable at the call site) or **keyed** mode. Pinned by `verify-is-anchor-relative` (unkeyed full-rewrite passes self, caught by anchor — **by design**, so nobody files it as a bug) and `keyed-chain-detects-forgery` (keyed detects an edit without an anchor).
2. **Chaining introduces append-ordering + startup-head-integrity deps.** *Concurrency:* in-process appends are atomic by **synchronous single-threaded execution** (no `await` in the body → no interleave; no mutex — a mutex over sync code is theater; corrected from the initial "mutex" answer). A cross-process fork is **detected** by `verify()`, not silently merged (CAS-for-the-log = fast-follow). Pinned by `concurrent-appends-linear-chain` (genuinely overlapped, asserts no duplicate `seq`). *Torn head:* a partial last line (crash mid-write) **fails loud** (`onAuditFailure`) and restarts as a fresh genesis whose **discontinuity is visible in `verify()`** — never a silent clean slate that would let a truncate-then-trigger-recovery attack hide the gap. Pinned by `torn-head-fails-loud`.

**Scope (locked with Opus).** Integrity core only. **Fast-follows:** log **rotation** (cross-segment chain linkage designed-in: a new segment's genesis `prev` = prior head), and **cross-process-log CAS** (mirroring the ledger). Kept separate so the security-critical part gets undiluted adversarial review.

**Status.** Built + green (default gate **127** = 121 + 6 audit tests; tsc clean; **0 runtime deps**; no-egress static proof intact — `chain-hasher.ts`/`hash-chain-log.ts` import only `node:crypto`/`node:fs`; e2e unchanged 12 + teeth). Requirements-first: AUDIT-01..03 (traceability green). Failing-test-first: stubbed `verify`/`load`/`onAuditFailure` → 5 security assertions RED (concurrency green from the start — sync-serialization regression guard) → implemented → GREEN. Held for Opus's adversarial pass before merge — he'll rewrite the whole log against a pinned anchor, race two genuinely-overlapped appends, and truncate-then-trigger a torn-head recovery to see if the gap hides.

### D-037 — the publishable artifact: a frozen, single-entry, dist-only package (built, not published)
**2026-07-17 · Accepted (packaging slice; "don't break userspace" applied to the package boundary)**

**Context.** The package had no installable shape — no entry point, no `exports`/`main`/`types`, and a `tsc` that emitted `dist/src/…` + `dist/test/…`. This slice builds and *proves* a clean installable artifact, deliberately **before** publishing: the version is **held at 0.1.4** (a semver number is a promise made at publish; building the artifact makes no promise). The `0.2.0` bump + `npm publish` follow the property-test pass — build-the-artifact → property-test the pinned surface → then publish.

**Decision — a deliberately minimal, frozen public surface reachable by one path only.**
- **Underexport as the governing principle (PKG-01).** What a package exports is a forever contract; the asymmetry is one-directional (adding an export is a conventionally-compatible minor; removing/renaming is a break), so the barrel ships the *minimum* — 12 values + the type/seam set — and a build-failing **freeze test** (`public-surface-is-frozen`) makes any surface change a deliberate, reviewed act. On reflection two symbols were trimmed *further* (`FileDecisionLog` — superseded by the tamper-evident `HashChainDecisionLog`; the low-level brand constructors — `parsePolicy`/`loadPolicyFile` are the sanctioned construction boundary); both are non-breaking to add back when a real consumer needs them, which is exactly why trimming now is the safe default.
- **The barrel is the SOLE path — for types as well as runtime (PKG-02/04).** `exports` exposes only `.` (+ `./package.json`), no subpath, no wildcard: a deep import throws `ERR_PACKAGE_PATH_NOT_EXPORTED` at runtime, and under `nodenext`/`bundler` resolution `tsc` honors the same map so an internal symbol can't be type-resolved either. Same "make the alternate route absent by construction" discipline as the signer allowlist (D-029).
- **`dist`-only, map-free build (PKG-03/05).** `tsconfig.build.json` emits JS + declarations from `src` only, rooted at `src`; `files: ["dist"]`. **No source/declaration maps** — a shipped map would dangle against un-shipped source or embed an absolute local path (`/home/…`), so shipping none makes the leak *impossible* rather than merely absent. Keeps the whole test tree (whose e2e code opens sockets) out of what a stranger installs.

**Two refinements to the review guidance, made explicit (not slipped past).**
1. **Type-channel block via a test, not a declaration bundler.** The robust-but-heavy option (bundle to one `.d.ts` so no internal file exists) would add a build-time dependency (esbuild/rollup) to a *security library's* publish slice — the exact supply-chain surface kept at zero. Instead: per-file declarations ship, and `pack-install.e2e.test.ts` **proves** a deep type-import fails `tsc` under `nodenext`. The residual — a *legacy*-resolver consumer *deliberately* deep-importing an internal — is the operator dismantling their own guard, which is out of the threat model (the guard defends against a lied-to agent and a hostile server through *data*, not against the trusted operator's own import statements).
2. **Dist-only ⇒ no maps.** "dist-only *and* source maps for go-to-definition" is internally contradictory (a map needs the source it points at). Chose dist-only + no maps; go-to-definition lands on the `.d.ts`, standard for a compiled library, and PKG-05's path-leak check becomes trivially strong.

**Validation (the teeth).** `pack-install.e2e.test.ts` packs a real tarball, unpacks into a throwaway `node_modules`, imports **only** the barrel and runs `snapshot()` + the binding, then attacks: runtime deep-import throws, type deep-import fails `tsc`, and the shipped file list has no `.ts`/`.map`/test/secret. The packaging analog of the ACCT-05 cross-process smoke test.

**Status.** Built + green (default gate **132** = 127 + 5 PKG hermetic tests; tsc clean; **0 runtime deps**; e2e **15 + skips**, pack teeth included; matrix **49/49** verified, CI-staleness-guarded). Requirements-first: PKG-01..05. Failing-test-first: barrel/config/manifest absent → hermetic suite + pack teeth RED → implemented → GREEN. Version **held at 0.1.4** — not published. Held for Opus's adversarial pass before merge: install the tarball and try to reach an internal by *any* path (runtime, type, `require`/`exports` edge), and read every shipped file for a `.ts`, an e2e artifact, a token, or a home-directory path.

### D-038 — P0 fix: the CAS is immune to version-number reuse (the cleanup/CAS ABA)
**2026-07-17 · Accepted (P0 over-allow fix; found by the pack-install test's contention, proven deterministically)**

**Context.** The publishable-artifact slice (D-037) added a pack-install e2e whose CPU contention turned `main`'s CI red: the cross-process smoke **honesty gate** (asserts exactly the cap is admitted) got **11 of 10**. Investigation proved — *deterministically*, not by timing luck — a real **ABA** in the ACCT-05 CAS store (shipped in v0.1.4): `compareAndSave` commits by `link()`-ing to `<ledger>.v<expected+1>`, treating **EEXIST** as the sole conflict signal, while `cleanup` (`KEEP_VERSIONS=3`) **deletes old version files, freeing their numbers**. A writer stalled mid-commit while holding a now-old `expected` targets a version number `cleanup` reclaimed; its `link()` into the freed hole *succeeds* → EEXIST is no longer a reliable CAS signal → the stale writer commits an **orphan below the true max**. `load()` reads the max and ignores the orphan, so the **durable ledger stays correct** — but `compareAndSave` returned `true`, so `authorize()` returns a **spurious `allow`**: one real over-cap payment signs in production. **Bounded** (one per occurrence, no compounding, ledger uncorrupted), but a genuine **ACCT-05 violation**. Caught **before publish, before mainnet, before funds** by an honest test written to prove something else; `main` was left red and the mechanism proven against our own work rather than buried.

**Decision — a derived-floor guard (ACCT-07).** `compareAndSave` rejects (conflict) any target `<= reclaimedFloor()`, where **`reclaimedFloor() = highestVersion() - KEEP_VERSIONS`** — *derived, not stored*. Because `cleanup` runs every commit and never deletes the top `KEEP_VERSIONS`, every number at/below that floor has been freed and everything above still exists; a target there is a stale writer whose number was reclaimed. Applied **pre-link** (fast reject) *and* **post-link** (the load-bearing half: re-check after the `link()` to close the check→link race, valid because `highestVersion()` is monotonic-nondecreasing). Retracting a below-floor orphan is **safe — never a double-count** — because no reader `load()`s below the max, so nothing was ever built on it; we retract only proven orphans, never a live tip. A notable robustness property (surfaced in review): **C is correct even if `cleanup` lags, fails, or is disabled**, since the floor is derived from the max, not from what cleanup actually deleted.

**Rejected alternatives.**
- **A — remove `cleanup` (no deletion → no reuse).** Obviously correct but unbounded disk growth: every `authorize` reaching `compareAndSave` writes a version file *including the deny-path window-persist*, so a capped wallet under a hammering agent churns files fastest exactly under the contention we harden. Rejected.
- **B — tombstone `cleanup` (truncate to empty, keep the name).** No reuse, bounds *space* — but one empty inode per commit forever (inode exhaustion), **and** it turns a benign vanished-mid-read (`ENOENT`→re-enumerate) into a parse-error→deny (a spurious-deny liveness regression). Rejected.

**Tests (foundation, not just symptom — the review condition).**
- **Deterministic regression, guaranteed non-vacuous (RED without the fix / GREEN with):** `cas-rejects-stale-writer-after-cleanup` (a stalled writer into a reclaimed hole → must conflict) and `cas-two-stale-writers-same-reclaimed-hole-both-rejected` (both, not just the first).
- **Depth × contention e2e** (`cross-process-stress.e2e.test.ts`): real processes at depth assert **no over-allow**, plus a separate **prober process** asserting `highestVersion()` (via `load().version`) **never goes backwards** under concurrent commit+cleanup — the monotonicity claim the floor guard rests on, **proven mechanically** rather than by prose. The lesson behind this — property/stress at depth vs the real store is a required layer for concurrency-core slices, and GC-adjacent-to-an-invariant is *part of* the invariant — is captured in [TEST_PLAN.md](../TEST_PLAN.md) §9.

**Status.** Fixed + green (default gate **134**; matrix **50/50**; e2e **18 + skips** with teeth — the smoke honesty gate now passes with teeth on, the exact CI failure that started this). Prioritized **ahead of the npm-publish pipeline**, as a proven over-allow in the anti-drain core is P0. Held for Opus's adversarial pass before merge: re-run the reclaimed-hole sequence against the guard (must now conflict), race two stale writers into one hole, and try to make `highestVersion()` under-report by interleaving `readdir` against a mid-flight commit+cleanup.

### D-039 — uniform directory-permission gates (world-writable dir refusal: CONF-03 + ACCT-08)

**Decision.** Refuse a **world-writable directory** holding a security-sensitive file — the policy file (**CONF-03**, `config.dir_world_writable`) and the spend ledger (**ACCT-08**, refused per-`load()` → `state.load_failed`). Same predicate as the file-level gates (`modeIsWorldWritable`, so win32-guarded, PLAT-01), applied to the *directory's* mode. Closes the vector left deferred by D-034's file-only scope.

**Why the file check alone was half a measure.** Create/delete/rename of a directory's entries is governed by the **directory's** write bit, not the file's. A `0o600` file in a world-writable dir is still: (policy) **replaced** — an attacker swaps `policy.json` for a permissive one, itself `0o600`, passing CONF-01; (ledger, worse) **plant** — an attacker creates a forged higher-version `ledger.v999` (0o600) that `highestVersion()` picks, passing ACCT-06 → forged spend state → drain. The ledger plant is the real drain path, which is why ACCT-08 checks per-`load()` (catching a post-startup `chmod` + plant), not only at startup.

**Sticky-blind (the design fork, converged with Kevin).** The initial instinct was to be *lenient* on world-writable+sticky dirs (`/tmp` = `0o1777`). On analysis the sticky bit only prevents **delete/rename of other users' files** — it does **not** prevent **create**. So sticky protects the policy replace-vector but **not** the ledger plant-vector (an attacker can still create `ledger.v999` in a sticky dir). Special-casing sticky would add complexity *and* leave the ledger hole open — the worst combination, and a blessed hole. So the refusal is **sticky-blind**: refuse a world-writable dir, sticky or not. Cost: refusing `/tmp`-style homes for security files — which is correct guidance anyway (tests use `mkdtemp` `0o700` subdirs, so no false-positive; the check is on the file's *immediate* parent only, never ancestors, so `/tmp` being the grandparent is irrelevant). Captured in the test `ledger-refuses-world-writable-dir-even-if-sticky`.

**World-only, not group (rationale made explicit, per Kevin).** Every gate (file and dir) is scoped to the **world-write bit only**. The line is the trust model — v1 is **single-tenant isolation** (THREAT_MODEL §3): a UNIX group is a set the user/admin configured and *trusts* (inside the boundary); *world* crosses it. And "world-writable" is a judgment-free bright line, whereas "your group is untrusted" is the guard judging the user's environment — policy in the guard, the mechanism-not-policy razor (a `staff`/deploy/CI group is routinely intentional). A genuinely shared-group host is the deferred **shared-multi-user** posture — met by a keyed/ACL'd store through the topology-agnostic `SpendStore` seam — not a widening of a mode-bit check.

**Placement guidance made concrete (per Kevin — no more vague "somewhere").** We gave `%LOCALAPPDATA%` for Windows but nothing concrete for POSIX. Now stated (REQUIREMENTS shared note, THREAT_MODEL §3): keep the policy/ledger in an **owner-only `0o700` directory, e.g. under `$HOME` (`~/.local/share/x402-spendguard/`)**; the guard refuses a world-writable directory so a shared location fails loud.

**By construction, not by call-order (CHENG NO-GO → hardened, Kevin's line).** The first cut placed the ledger dir check only in `load()`, so the refusal held only because the guard calls `load()` before `compareAndSave()`. CHENG's adversarial pass (and Kevin's insistence) rejected that: locating a security invariant in *caller call-order* is the exact fragility class the CAS-ABA P0 taught us to avoid ("safe because a cooperating caller does the right thing" is not safe — the door is open, no one has walked through it yet). `FileSpendStore` is a public seam; a future adapter/refactor calling `compareAndSave()` without a prior `load()` would reopen the plant vector. Fix: extract `assertDirTrusted()` and call it at the top of **both** `load()` and `compareAndSave()` — the refusal is now a property of the store, held on every trust-taking op regardless of caller.

**Tests (failing-first).** `rejects-world-writable-policy-dir` (CONF-03), `ledger-refuses-world-writable-dir` (ACCT-08), `ledger-refuses-world-writable-dir-even-if-sticky` (the sticky-blind fork), and `compareAndSave-refuses-world-writable-dir` (the by-construction guarantee — hits the second entry point directly, bypassing `load()`; mutation-proven RED without the `compareAndSave` guard). All RED before the checks, GREEN after; all self-skip on win32 (PLAT-01), like the file-level perm tests. No new predicate: every call site reuses `modeIsWorldWritable`.

### D-040 — the shipped dashboard is a dump-file exporter + a static viewer, never a socket

**Decision.** The shipped dashboard (chapter target **0.4.0**) is (1) a serialized **export** the guard *writes* (`serializeSnapshot` → `writeSnapshotExport`) and (2) a single **dependency-free static HTML viewer** that *reads* it. No `node:http`, no loopback endpoint, no server the guard runs, no `fetch`. Three converge-first slices: **D1** serializer + `0o600` writer, **D2** operator-supplied head anchor + a `verify` CLI, **D3** the math-free static viewer. Full design: [design-dashboard-export.md](design-dashboard-export.md).

**Why no socket (the crown jewel).** The statically-provable **no-egress** guarantee (PRIV-01/T12) rests on the core importing nothing socket-capable. A loopback endpoint breaks that proof and becomes a *local* read surface the guard owns and must defend forever (other users/processes on a shared host can hit `127.0.0.1`). A dump-file + bring-your-own-viewer keeps that surface out of our supply chain entirely — the operator points their own already-trusted static server at the file if they want it live. The **shipped-vs-prop distinction does not change the egress calculus**; "it's a real dashboard now" is not a reason to open a socket. The data is pull-shaped and point-in-time (a snapshot is already a pull, `verify()` a batch op), so nothing about it wants a live query. The viewer stays **single-file, zero-dependency** for the same reason: a dependency tree can phone home, reopening egress by the back door.

**The wire format's one hard job — silent money corruption, defended by construction.** JSON has no bigint and base-unit amounts routinely exceed 2⁵³, so a naive `Number(amount)` would render a confident lie. Money serializes as a **tagged envelope** `{ "$b": "<base-units>", "text": "<human>" }`: a naive `JSON.parse` yields an *object* (so `Number()` → `NaN`, loud), the shipped **reviver** (`parseSnapshotExport`) recovers an exact `bigint` via `BigInt`, and `text` is **pre-rendered in trusted Node** by the >2⁵³-tested `renderAmount` so the viewer displays text and does **no money math at all** — the reference consumer is safe by construction, not by discipline. Envelope protects the format; pre-render protects the reference impl; reviver serves the programmatic path. Timestamps stay plain numbers (safe-range). (EXPORT-01.)

**The head anchor — honest about a box we don't control (D2).** `verify(expectedHead)` only catches a full log rewrite if the head is pinned where the log-writer's attacker can't also rewrite it — and the guard **cannot find such a spot on a box it doesn't control**, so it doesn't pretend to. A same-directory, same-permission sibling file is *not* an anchor (the attacker rewrites it too — theater). The honest single-tenant answer: the guard **emits** the head; the anchor is **operator-supplied** (a secrets store, a different host, a printed value), and `verify` checks against what they supply; for keyed mode the **key is operator-held**, never in the export or viewer. The static viewer *displays* the head labelled "compare to your last-known-good" and never claims an authority a static file can't have. (D2, next slice.)

**The export is the sensitive artifact now.** It materializes the full spend posture **plus the counterparty graph** (`byDomain[].origin`) into a file *designed to be moved*, so it is **ledger-grade sensitive**: written `0o600` (PRIV-04), atomically (temp + rename) so every write lands owner-only regardless of a prior file's mode; docs say do not commit/log/serve it without ledger-grade care. Redaction of the counterparty origin is an **opt-in caller option** (default lossless — never hide the owner's own data from the owner; mechanism-not-policy). (EXPORT-02/03.)

**Rejected.** A loopback HTTP endpoint (reopens the egress surface the whole project is built to prove absent). A bundled viewer with a dependency tree (a dep can phone home — egress by the back door). Bare-string or suffix money encodings (a naive `Number()` still corrupts/half-corrupts; only the object envelope makes the first touch fail loud). A guard-written "safe" anchor file (theater on a box the attacker already writes to).
