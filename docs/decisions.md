...
- **L2 — world-writable permission check on the ledger *and* the new log file.** A one-line mirror of CONF-01, but pulling a deferred item forward is a scope call → converge first. Flagged, not done.
- **Silent audit loss.** A failing sink is swallowed (correct per FAIL-03: must not flip the decision) but currently *silently* — an attacker who breaks the log blinds the audit trail while payments proceed. Surfacing audit failures (without flipping the decision — e.g. an injected error callback) is a proposed enhancement, converge-first. Noted here rather than shipped silent.
