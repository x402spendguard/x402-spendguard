...
**Context.** The decision log (AS3) is the forensic record and the read-API's tightest rung ([[dashboard-positioning]]). This slice makes it **tamper-evident**: per-entry `seq` + hash chain (each record commits to the prior). Designed through the ratified trust model (D-035 doc, THREAT_MODEL §3) and Opus's shape review, which added two load-bearing tightenings before the build.
