...
1. **It is not more enforceable than any in-process guard.** An agent whose code an attacker controls calls the signer directly and bypasses it. The guard gives you *visibility into what is about to be signed*, not *authority over whether it is signed*. It stops an honest agent that has been lied to — prompt injection, a compromised tool, a runaway loop — not an agent whose code is owned. Raising that ceiling (a key-holding local daemon; on-chain delegation) is the roadmap, not this version.
2. **Cross-process spend accounting is enforced and validated — on one host — by a compare-and-swap store.** Multiple processes on **one host** sharing a wallet's local-disk ledger are serialized: the store commits only if the ledger hasn't moved since it was read (an atomic `link()`-based compare-and-swap), so a losing writer is *told* it lost and re-evaluates rather than silently overwriting. A real N-process race (`test/e2e/cross-process-smoke.e2e.test.ts`) exercises this: against a cap that admits exactly N payments under far heavier demand, **exactly N are admitted and the durable ledger records every one** — no lost updates. The store also **fails closed on any filesystem it can't prove honors atomic exclusive-create**: NFS and SMB are refused by name, and anything that fails a live exclusive-create probe is refused too. **The honest boundary:** sharing one ledger across *separate hosts* needs a networked filesystem — exactly the case the store won't trust — so multi-host is out of scope for the *file* store. That deployment wants an external CAS store (a database, a Durable Object — a future adapter over the same versioned seam), not a shared file. (ACCT-05, D-031, D-032.)
3. **Spend state must live on a store the guard can prove atomic — unsupported setups fail *loud*, not silent.** The bundled file store needs a writable, persistent path that honors atomic exclusive-create, and **refuses to start** on one that doesn't rather than under-enforce silently. A truly **ephemeral or stateless** runtime — no durable store anywhere — genuinely cannot enforce *cumulative* caps, because there is nowhere to keep the running total; the honest answer is to give the guard a durable store (the store interface is a topology-agnostic versioned contract — a database, a Durable Object, etc.) or run one long-lived process with a real disk. **Do not expect cumulative caps to hold with no durable store.** At rest, the ledger is created **owner-private** (`0o600`), and a **world-writable** ledger is **refused** before it is trusted — a tampered ledger could reset spend — the same posture as the decision log (ACCT-06/PRIV-04). This applies to the ledger *file*; don't place it in a world-writable *directory*. **On Windows**, these POSIX mode-bit checks don't apply — Windows uses ACLs, not mode bits — so they are **skipped** to avoid misfiring (PLAT-01); rely on **NTFS ACLs** instead by keeping the ledger, policy, and log under your user profile (`%LOCALAPPDATA%`), where inherited ACLs already restrict them to your account.
4. **Write-ahead accounting over-counts on downstream failure.** Spend is recorded before the payment settles (so a crash never under-counts) — but a payment the facilitator later rejects still consumes budget until a settlement-reconciliation path exists (a later version). A run of failed payments can walk you into your cap early.
