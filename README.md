...
2. **Cross-process spend accounting is enforced by a compare-and-swap store — with one honest validation caveat.** Two processes sharing one wallet's ledger are now serialized: the store commits only if the ledger hasn't moved since it was read (an atomic `link()`-based compare-and-swap), so a losing writer is *told* it lost and re-evaluates instead of silently overwriting. The store also **refuses to run** (fail-closed) on a filesystem it can't prove honors atomic exclusive-create, and refuses known-unsafe network mounts (NFS/SMB) outright. **Caveat we won't paper over:** the CAS *logic* is proven, but a genuine **two-process (ideally two-host) run on your target filesystem** is the final check before you rely on it in a multi-process deployment — until you've done that, one instance per wallet is still the conservative choice. (ACCT-05, D-031.)
3. **Spend state must live on a store the guard can prove atomic — unsupported topologies fail *loud*, not silent.** The bundled file store needs a writable, persistent path that honors atomic exclusive-create; it **refuses to start** on one that doesn't, rather than silently under-enforcing. A truly **ephemeral/stateless** runtime (no durable store anywhere) genuinely cannot enforce *cumulative* caps — there's nowhere to keep the running total — so the honest answer is to give the guard a durable store (the store interface is a topology-agnostic versioned contract: a database, a Durable Object, etc.) or run one long-lived process with a real disk. **Do not expect cumulative caps to hold with no durable store.** Our rule: an unsupported deployment fails *loud*, not silently mis-enforcing.
