...
- **At-rest hardening is *opportunistic*, not the trust model.** Protecting the ledger and log *files* — and the directories that hold them — from a co-located reader/writer (the POSIX permission gates today, ACCT-06/CONF-01/CONF-03/ACCT-08/PRIV-04; a tamper-evident audit log next) is a **defense-in-depth layer added where the platform offers a meaningful mechanism** — POSIX mode bits on Unix (keep the policy/ledger in an owner-only `0o700` directory, e.g. under `$HOME`), NTFS ACLs on Windows via file placement (`%LOCALAPPDATA%`). Where a platform offers none (Windows synthesizes mode bits, so the gates are skipped — PLAT-01; a stateless FS has no persistent perms at all), the guard **degrades to the isolation boundary, never to a false sense of protection** — it does not pretend a check ran that did not.
...
- **ASM2 — The local clock is roughly correct.** BIND-03's window bound depends on it. A large clock skew weakens the bound; CLOCK-01 constrains the failure to the safe direction.
