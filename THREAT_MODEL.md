...
- **ASM6 — Deployment is a single long-lived process with a writable, persistent, local ledger.** The bundled `FileSpendStore` assumes a path that is writable, survives restarts, and is not shared by another process. Each assumption has a failure mode, and today each fails **silently** — which is the actual defect, not the missing feature: (a) *read-only filesystem* (hardened containers, `readOnlyRootFilesystem`, distroless) — the store cannot persist at all; (b) *ephemeral storage* (serverless per-instance `/tmp`, wiped between invocations) — every cold start begins with empty spend state, so cumulative caps reset and stop enforcing, a fail-**open** on the anti-drain guarantee that anyone able to spin up fresh instances can trigger; (c) *shared storage across processes* — the ACCT-05 lost-update. **Governing principle: an unsupported deployment topology must fail _loud_, not silently mis-enforce.** v1 documents these and requires one long-lived process with a real disk; *detecting and refusing* an unsupported store at startup — and supporting richer multi-process/serverless topologies at all — are tracked for the durable-state slice (see [decisions D-021](docs/decisions.md)). Which topologies to honestly support is a prerequisite of that slice, decided before its code, not an afterthought.
