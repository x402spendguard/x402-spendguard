...
| **EXPORT-01** | `serializeSnapshot()` produces a **lossless, corruption-resistant** export (D-040). Money is a **tagged envelope** `{ "$b", "text" }`: a naive `JSON.parse` yields an object so `Number()` is `NaN` (loud) rather than a silent >2⁵³ corruption, `parseSnapshotExport` (the shipped reviver) recovers the **exact `bigint`** via `BigInt`, and the pre-rendered `text` (via the >2⁵³-tested `renderAmount`) matches the raw value — so the reference viewer displays text and does no money math. Timestamps are plain numbers. | (design) | `export-is-lossless-and-corruption-loud`, `export-handles-null-caps-and-no-display` |
| **EXPORT-02** | The dumped export is **ledger-grade sensitive** (full spend posture + the counterparty graph, in a file designed to be moved), so `writeSnapshotExport()` writes it **owner-only `0o600`** (PRIV-04), atomically (temp + rename) so every write lands owner-only regardless of a prior file's mode. POSIX-only (Windows ignores the mode; PLAT-01). | T15 | `export-file-is-owner-only`, `write-is-atomic-overwrite-stays-owner-only` |
| **EXPORT-03** | Redaction of the counterparty `origin` is an **opt-in caller option** (`redactOrigin`), default **lossless** — the export never hides the owner's own data from the owner (mechanism-not-policy); a caller exporting to a less-trusted place supplies the redaction. | (mechanism-not-policy) | `export-redaction-is-opt-in` |
| **ANCHOR-01** | The audit head anchor is **operator-supplied, never self-attested** (D-040). A full self-consistent rewrite passes self-verify (documented) but is caught by `verify` against a head the operator **pinned out-of-band**, or a **keyed** hasher whose key the operator holds. The serialized audit export (`serializeAudit`) reports only its own `head` + `selfConsistent` — **no** "anchored/verified" verdict, because a file an attacker could rewrite cannot make one about itself. A same-directory sibling file is not an anchor. | T14 | `operator-anchor-catches-a-rewrite-self-verify-misses`, `keyed-mode-catches-a-full-rewrite-without-an-anchor`, `audit-export-self-reports-head-not-an-anchored-verdict` |
| **VIEW-01** | The shipped viewer (D-040, D3) is a **single dependency-free HTML file** that renders a dumped export. It **imports nothing and cannot egress** — no `fetch`/socket/`<script src>`/external asset — so the no-egress proof (PRIV-01) holds at the edge too. It is **math-free**: it never touches the `$b` base-unit field and parses no number out of money, so it **displays the pre-rendered `text`** (a >2⁵³ amount renders exactly, never rounded) and is structurally unable to corrupt an amount on screen. It **escapes** attacker-influenced counterparty strings (the domain `origin`). The audit `head` is shown as a value to **compare** out-of-band, never as a "verified" verdict a static file cannot make. | (design) | `viewer-imports-nothing-and-cannot-egress`, `viewer-is-math-free-never-touches-base-units`, `viewer-renders-a-large-amount-exactly-from-text`, `viewer-escapes-untrusted-counterparty-strings`, `viewer-audit-head-is-compare-not-verified` |
...
| ID | Requirement | Threat | Test |
|----|-------------|--------|------|
| **POL-01** | The enforcement path contains **no security-deciding literal** — every threshold, list, and tolerance is read from `Policy`. Shipped defaults (clock skew, the cross-origin flag) live in a **readable starter policy** (`policy.example.json`, shipped as code via `STARTER_POLICY_JSON`; see ONBOARD-01), never as constants in code. | (mechanism-not-policy) | `no-deciding-literals-in-core` |
| **ONBOARD-01** | A shipped, readable **starter policy** exists and reaches an `npm install` user as code — `STARTER_POLICY_JSON` plus `writeStarterPolicy`, which writes it **owner-only** (PRIV-04; the allowlist is the counterparty graph) and **refuses to overwrite** an existing file (fail-closed against clobbering a live policy). The repo-root `policy.example.json` is byte-identical (drift-gated). This is the artifact POL-01 references. | (onboarding) | `starter-policy-file-exists`, `write-starter-refuses-overwrite`, `write-starter-is-owner-only` |
| **ONBOARD-02** | The starter **fails loud when copied unedited** — its placeholder payee / token / caps are rejected by `parsePolicy` with a specific reason, so a new user gets a legible error rather than a silent deny-all; and with placeholders filled it still parses (not stale against the `Policy` type). | (onboarding) | `starter-fails-loud-and-is-not-stale` |
| **ONBOARD-03** | `describePolicy` **echoes every cap in human units** when the user declared its `display` decimals — so the one authoring error that fails *open* relative to intent (an off-by-a-zero cap) is **visible** — and **degrades to exact base units**, never a guessed rendering, when decimals are absent. It renders the user's own declarations only: no token registry, no lookup, no egress. Ships to an `npm install` user. | (onboarding) | `echo-renders-caps-in-human-units`, `echo-without-decimals-shows-base-units-not-a-guess` |
| **DISP-01** | `display` metadata (decimals / symbol) is **display-only and structurally non-enforcing**: it lives OUTSIDE `Policy` (`parsePolicy` ignores it; the enforcement path never receives it), so it cannot affect a decision by construction. A wrong or misleading `symbol` changes only a label. | (mechanism-not-policy) | `display-never-affects-a-decision` |
| **ONBOARD-04** | The user-facing **deny-reason legend** (`docs/reason-codes.md`) is GENERATED from the reason registry and enforced complete **both directions**: every registered code has a legend row (`legend-documents-every-code`), and every registered code is actually emitted in `src/` so no row describes a code the guard never produces (`every-registered-code-is-actually-emitted`). A CI staleness gate (`npm run reason-codes` + `git diff --exit-code`) stops the committed legend drifting or being hand-edited. Each row carries *where you encounter it* (load / payment / read-api). | (observability) | `legend-documents-every-code`, `every-registered-code-is-actually-emitted` |
| **LINT-01** | `lintPolicy` **advises on structural contradictions** in a parsed policy — a **dead cap** (caps not in `perRequest ≤ perDomain ≤ global` order, so the outer cap can never bind) and an **unpayable allowlist entry** (a destination allowlisted on a chain with no caps denomination, so CAP-05 denies every payment to it) — plus the mirror **unreachable denomination**. It reports *contradictions*, never *choices* (mechanism-not-policy: it never says a cap value is wrong, only that a cap is structurally inert). **Advisory-only**: pure, no egress, and it never gates enforcement — `parsePolicy` remains the trust gate. A clean policy yields zero findings (non-vacuity anchor). | (onboarding) | `lint-clean-policy-has-no-findings`, `lint-flags-a-dead-cap-when-perRequest-exceeds-perDomain`, `lint-flags-an-unpayable-allowlist-entry-with-no-caps-for-its-chain` |
