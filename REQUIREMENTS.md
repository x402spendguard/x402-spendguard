...
| **ACCT-08** | The `FileSpendStore` refuses to read from a **world-writable ledger directory** (checked per-`load()`, before any version file is trusted). A world-writable dir lets any local user **plant a forged higher-version file** that `load()` then picks — even at `0o600`, defeating ACCT-06's per-file check. **Sticky-blind:** the sticky bit stops delete/rename but not *create*, so it does not close the plant vector. World-only; POSIX-only (PLAT-01). | T15 | `ledger-refuses-world-writable-dir` |
